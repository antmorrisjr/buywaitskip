...
export default function CreatorsPage() {
  ...
  const avatarSize = isMobile ? 64 : 120;
  const nameFontSize = isMobile ? 13 : 22;
  const handleFontSize = isMobile ? 11 : 14;
  const statFontSize = isMobile ? 14 : 18;
  const statLabelSize = isMobile ? 9 : 10;
    ...
    <div style={{ minHeight: "100vh", backgroundColor: BG, overflowX: "hidden" }}>
      ...
      <div style={{ maxWidth: 1152, margin: "0 auto", padding: isMobile ? "32px 12px" : "48px 24px" }}>
        <div style={{ marginBottom: 40 }}>
          ...
          <h1 style={{ color: "white", fontSize: isMobile ? 32 : 42, fontWeight: "bold", margin: "0 0 12px" }}>Our Creators</h1>
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: gridCols, gap: isMobile ? 10 : 20 }}>
            {creators.map(creator => (
              ...
              >
                <div
                  style={{ backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: isMobile ? 12 : 24, cursor: "pointer", transition: "transform 0.2s, border-color 0.2s", height: "100%" }}
                ...
                >
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: isMobile ? 8 : 12 }}>
                    {creator.avatar_url ? (
                      <img
                        src={creator.avatar_url}
                        alt={creator.name}
                        referrerPolicy="no-referrer"
                        style={{ width: avatarSize, height: avatarSize, borderRadius: "50%", objectFit: "cover", border: "2px solid rgba(255,255,255,0.1)", flexShrink: 0 }}
                      />
                    ) : (
                      <div style={{ width: avatarSize, height: avatarSize, borderRadius: "50%", backgroundColor: `hsl(${creator.name.charCodeAt(0) * 137 % 360}, 50%, 25%)`, border: "2px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: isMobile ? 24 : 36, fontWeight: "bold", color: `hsl(${creator.name.charCodeAt(0) * 137 % 360}, 70%, 70%)`, flexShrink: 0 }}>
                        {creator.name.charAt(0).toUpperCase()}
                      </div>
                    )}
                    <div style={{ minWidth: 0, width: "100%" }}>
                      <p style={{ color: "white", fontWeight: "bold", fontSize: nameFontSize, margin: "4px 0 2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{creator.name}</p>
                      <p style={{ color: "rgba(255,255,255,0.4)", fontSize: handleFontSize, margin: "0 0 8px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{creator.handle}</p>
                    </div>
                    <div style={{ display: "flex", gap: 6, width: "100%" }}>
                      <div style={{ flex: 1, textAlign: "center", padding: isMobile ? "6px 2px" : "8px 4px", backgroundColor: "rgba(0,230,118,0.08)", borderRadius: 8, minWidth: 0 }}>
                        <p style={{ color: GREEN, fontSize: statFontSize, fontWeight: "bold", margin: 0 }}>{creator.review_count}</p>
                        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: statLabelSize, margin: "2px 0 0", textTransform: "uppercase", letterSpacing: "0.05em" }}>Reviews</p>
                      </div>
                      <div style={{ flex: 1, textAlign: "center", padding: isMobile ? "6px 2px" : "8px 4px", backgroundColor: "rgba(255,215,64,0.08)", borderRadius: 8, minWidth: 0 }}>
                        <p style={{ color: GOLD, fontSize: statFontSize, fontWeight: "bold", margin: 0 }}>{formatSubs(creator.subscriber_count)}</p>
                        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: statLabelSize, margin: "2px 0 0", textTransform: "uppercase", letterSpacing: "0.05em" }}>Subs</p>
                      </div>
                    </div>
                    {!isMobile && creator.genre_tags?.length > 0 && (
